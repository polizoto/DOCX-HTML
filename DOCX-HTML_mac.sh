...
# Different commands than PC Script
...
sed -i '' '/~%~@/s/[[:digit:]]/@&/g' ./"$baseName"/"$baseName".html
...
perl -pi -e 's/&amp\; /\& /g' ./"$baseName"/"$baseName".html

perl -pi -e 's/<mi>&amp\;<\/mi>//g' ./"$baseName"/"$baseName".html

perl -pi -e 's/\\\& /\& /g' ./"$baseName"/"$baseName".html

# Correct code for displaying Desmos graphs

perl -pi -e 's/<p>&lt;iframe/<iframe/g' ./"$baseName"/"$baseName".html

perl -pi -e 's/&gt;&lt;\/iframe&gt;<\/p>/><\/iframe>/g' ./"$baseName"/"$baseName".html

perl -pi -e 's/frameborder=0//g' ./"$baseName"/"$baseName".html

perl -pi -e 's/500px/500/g' ./"$baseName"/"$baseName".html 

#

# Correct YouTube Embed Code

perl -pi -e 's/frameborder="0"/style="border: 1px solid #ccc"/g'  ./"$baseName"/"$baseName".html 

#

# Correct Amara Embed Code

perl -pi -e 's/<p>&lt;div class="amara/<div class="amara/g' ./"$baseName"/"$baseName".html 

perl -pi -e 's/"null"&gt;&lt;\/div&gt;<\/p>/"null"><\/div>/g' ./"$baseName"/"$baseName".html

perl -pi -e 's/<p>https:\/\/amara.org\/en\/videos\//<div class="amara-embed" data-hide-logo="true" data-width="560px" data-initial-language="en" data-show-subtitles-default="true" data-video-id="/g' ./"$baseName"/"$baseName".html

# Problematic - did not work on macOS...trying Perl

#sed -i '' 's/\/info\/.*\/<\/p>/" data-team="null"><\/div>/g' ./"$baseName"/"$baseName".html

perl -pi -e 's/\/info\/.*\/<\/p>/" data-team="null"><\/div>/g' ./"$baseName"/"$baseName".html
...
# Remove spurious closing </p> tag at the end of a \begin matrix LaTeX command
...
if 	java -jar ~/vnu.jar --filterpattern ".*Possible misuse of “aria-label.*" --filterpattern ".*The first occurrence of ID “E.*" ./"$baseName"/"$baseName".html 2>&1 | grep -q 'info warning:' ; then

if [ "$check" == "on" ]; then 

echo -e "\n\033[1;33mATTENTION:\033[0m These HTML validation \033[1;33mwarnings\033[0m were found in \033[1;32m"$baseName".html\033[0m:\n"

java -jar ~/vnu.jar --filterpattern ".*Possible misuse of “aria-label.*" --filterpattern ".*The first occurrence of ID “E.*" ./"$baseName"/"$baseName".html 2>&1 | sed -n -e 's/^\(.*":\)\(.*\)\(info warning: \)\(.*\)$/\2\4/p'

fi

java -jar ~/vnu.jar --filterpattern ".*Possible misuse of “aria-label.*" --filterpattern ".*The first occurrence of ID “E.*" ./"$baseName"/"$baseName".html 2>&1 | sed -n -e 's/^\(.*":\)\(.*\)\(info warning: \)\(.*\)$/\2\4/p' > ./display.txt
...
if 	java -jar ~/vnu.jar --errors-only --filterpattern ".*Duplicate attribute “class”.*" --filterpattern ".*Duplicate ID “E.*" ./"$baseName"/"$baseName".html 2>&1 | grep -q 'error:' ; then

if [[ "$check" == on ]]; then 
		
echo -e "\n\033[1;33mATTENTION:\033[0m These HTML validation \033[1;31merrors\033[0m were found in \033[1;32m"$baseName".html\033[0m:\n"
		
java -jar ~/vnu.jar --filterpattern ".*Duplicate attribute “class”.*" --filterpattern ".*Duplicate ID “E.*" ./"$baseName"/"$baseName".html 2>&1 | sed -n -e 's/^\(.*":\)\(.*\)\(error: \)\(.*\)$/\2\4/p'

fi

java -jar ~/vnu.jar --filterpattern ".*Duplicate attribute “class”.*" --filterpattern ".*Duplicate ID “E.*" ./"$baseName"/"$baseName".html 2>&1 | sed -n -e 's/^\(.*":\)\(.*\)\(error: \)\(.*\)$/\2\4/p' > ./display.txt
...
awk '/<main>/{p=1;next}{if(p){print}}' ./"$baseName"/"$baseName".html > tmp && mv tmp ./"$baseName"/"$baseName".html

#sed -i '' -n '/<\/main>/q;p' ./"$baseName"/"$baseName"_pdf.html
awk '/<\/main>/ {exit} {print}' ./"$baseName"/"$baseName".html > tmp && mv tmp ./"$baseName"/"$baseName".html
